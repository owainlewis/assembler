...
import { collectPatch, repositoryURL } from './gvisor/repository.js';

export default defineWorkflow({
  input: z.object({
    repo: z.string().default('owainlewis/assembler').transform(repositoryURL),
  ...
  async run(ctx) {
    ...
    try {
      ...
      await ctx.step('Clone latest default branch', async () => {
        await sandbox.exec(['env', 'GIT_TERMINAL_PROMPT=0', 'git', 'clone', '--depth=1', '--single-branch', '--', ctx.input.repo, '.']);
        revision = (await sandbox.exec(['git', 'rev-parse', 'HEAD'])).stdout.trim();
        ctx.output('Source', { repository: ctx.input.repo, commit: revision });
        // Fetch-only demo: no GitHub credentials and no usable push URL.
        await sandbox.exec(['git', 'remote', 'set-url', '--push', 'origin', 'DISABLED']);
      });
      await ctx.step('Run prompt', async () => {
        ...
        ctx.output('Agent response', result);
      });
      await ctx.step('Save patch', async () => {
        ...
        ctx.output('Patch', join(artifacts, 'changes.patch'));
      });
    } catch (error) {
      failure = error;
      throw error;
      ...
      try {
        ...
        else await ctx.step('Remove sandbox and clone', cleanup);
        ctx.output('Cleanup', 'Container and temporary checkout removed; logs and artifacts retained.');
      } catch (error) {
